...
//axeDiagLog = format["HL:HOUSE NUM:%1 (From Last Run) | Range:%2",_houseNum,_rng];
	...
	if(count _litHouses < _maxHouses)then{
		{	
			if(_currLighting < _maxHouses)then{
				...
				if((_lightPcnt * 2) > floor (random 100))then{//Randomness required or first nearest houses to player get lit in a block - Needs to be based on range !
				...
				//Randomly light second window(s) - Eventually all will be lit.. maybe ?
				if(42 > floor (random 100)) then{
				//Only light if available otherwise animation 'attempt' continues indefinitely..
				...
				_dir = getDir _x;
					...
					[_lmpCol,_brtns,_lmpCol,getPos _x,_dir,[0,0,-0.6]] call axe_newLightPoint;
	...
	if(count _litHouses > 0 )then{
		{
			...
			if(_plyr distance _x > _lpRange)then{//If outside create radius delete light that IS in house
